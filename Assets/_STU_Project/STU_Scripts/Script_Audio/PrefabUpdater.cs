...
{
    // 未完成，目前仍然要手動選enum
    const string _prefabPath = "Assets/Resources/SoundManager.prefab"; 
    ...
    {
        if (stage.assetPath != _prefabPath)
            return;

        GameObject prefab = PrefabUtility.LoadPrefabContents(stage.assetPath);
        if (prefab != null && prefab.TryGetComponent(out SoundManager soundManager))
        {
            string[] soundNameList =
                soundManager.SoundLibraries.Select(x => x.Name)
                .Concat(soundManager.RandomSoundLibraries.Select(x => x.Name))
                .Distinct().ToArray();
            string[] musicNameList = soundManager.MusicLibraries.Select(x => x.Name).Distinct().ToArray();
